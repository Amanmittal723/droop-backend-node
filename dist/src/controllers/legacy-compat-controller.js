...
exports.LegacyCompatController = void 0;
/**
...
class LegacyCompatController {
    ...
    constructor(dependencies) {
        ...
        this.apnsService = new apns_service_1.ApnsService();
    }
    async queryRows(sql) {
    ...
    fileFromRequest(request, field) {
        return request.files?.find((file) => file.fieldname === field);
    }
    async saveUploadedFile(baseDir, file, extFallback) {
        const fileName = `${(0, random_1.legacyRandomString)(10)}${node_path_1.default.extname(file.originalname) || extFallback}`;
        const diskPath = node_path_1.default.join(baseDir, fileName);
    ...
    async saveBase64File(baseDir, ext, encoded) {
        const fileName = `${(0, random_1.legacyRandomString)(12)}.${ext}`;
        await promises_1.default.mkdir(baseDir, { recursive: true });
        await (0, storage_1.writeBase64File)(node_path_1.default.join(baseDir, fileName), encoded);
        return fileName;
    }
    async userBusinesses(userId) {
        const rows = await this.queryRows(`SELECT business_id FROM user_business WHERE user_id='${escapeSql(String(userId))}'`);
    ...
    async profileBase(userId, includeLoginStats = false) {
        const row = await this.queryRow(`SELECT * FROM user_master WHERE user_id='${escapeSql(userId)}'`);
        if (!row) {
            return null;
        }
        ...
        const collabs = await this.queryRow(collabSql);
        const posts = await this.queryRow(postSql);
        const likes = await this.queryRow(likeSql);
        const following = await this.queryRow(followingSql);
        const followers = await this.queryRow(followersSql);
    ...
    async updateProfileViewCount(userId, isCount = 0) {
        const row = await this.queryRow(`SELECT * FROM user_master WHERE user_id=${Number(userId)}`);
        if (!row) {
            return false;
        }
    ...
    async buildFollowerList(userId, loggedUserId, includePrices = false) {
        const loggedFollowingRows = loggedUserId !== userId
            ? await this.queryRows(`SELECT distinct(tr.following_id) FROM followers_master tr WHERE tr.followed_by='${escapeSql(loggedUserId)}'`)
            : [];
        ...
        const following = await this.queryRows(followingSql);
        let followers = [];
        ...
        followers = await this.queryRows(followersSql);
        return { following, followers };
    }
    async insertBulkUserContacts(userId, emails) {
        if (emails.length === 0) {
            return;
        }
    ...
    async insertNotification(text, sentTo, dual, sharedBy, dualId, notificationType) {
    ...
    async sendLegacyPush(request, sandbox) {
        const message = (0, legacy_request_1.getLegacyOptionalString)(request, "message") ?? "";
        const employeeIds = ((0, legacy_request_1.getLegacyOptionalString)(request, "emp_id") ?? "")
            .split("#")
            .map((value) => value.trim())
            .filter((value) => value.length > 0);
        const fromId = (0, legacy_request_1.getLegacyOptionalString)(request, "from_id");
        for (const userId of employeeIds) {
            if (fromId) {
                const muteRow = await this.queryRows(`SELECT user_id FROM mute_master WHERE user_id=${Number(userId)} AND mute_id=${Number(fromId)}`);
                if (muteRow.length > 0) {
                    continue;
                }
            }
            const user = await this.queryRow(`SELECT * FROM user_master WHERE user_id='${escapeSql(userId)}'`);
            const deviceToken = String(user?.device_token ?? "").trim();
            if (deviceToken.length === 0 || deviceToken === "NA") {
                continue;
            }
            try {
                await this.apnsService.sendPush(deviceToken, {
                    message,
                    dualId: (0, legacy_request_1.getLegacyOptionalString)(request, "dual_id"),
                    friendId: (0, legacy_request_1.getLegacyOptionalString)(request, "friend_id"),
                    userId: (0, legacy_request_1.getLegacyOptionalString)(request, "user_id"),
                    notificationType: (0, legacy_request_1.getLegacyOptionalString)(request, "notification_type"),
                    isCollaborate: (0, legacy_request_1.getLegacyOptionalString)(request, "is_collaborate") !== undefined
                }, sandbox);
            }
            catch {
    ...
    async fetchDualsForFeed(sqlWhere, sqlOrder = "ORDER BY date_time DESC", limitClause = "", includePrices = true, includeSaved = true, includeLoveCount = true) {
        let sql = "SELECT dp.*";
        if (includeSaved) {
    ...
    currentUserId = 0;
    async dualFeedRows(userId, options) {
        this.currentUserId = Number(userId);
        const followingRows = await this.queryRows(`SELECT following_id FROM followers_master WHERE followed_by='${escapeSql(userId)}'`);
        ...
        const blockedByRows = await this.queryRows(`SELECT user_id FROM blocks_master WHERE block_id='${escapeSql(userId)}'`);
        const blockedRows = await this.queryRows(`SELECT block_id FROM blocks_master WHERE user_id='${escapeSql(userId)}'`);
        ...
        const start = legacy_request_1.getLegacyOptionalString;
        let limitClause = "";
        ...
        sql += limitClause;
        const rows = await this.queryRows(sql);
        const totalRow = await this.queryRow(`SELECT count(dp.dual_id) as total FROM dualpost_master dp WHERE ${whereParts.join(" AND ")}`);
        return { rows, total: Number(totalRow?.total ?? 0), seed };
    }
    addContacts = async (request, response) => {
        try {
            const payload = typeof request.body === "object" && request.body ? request.body : {};
            const userId = String(payload.user_id ?? "");
            const rawEmails = payload.emails;
            const emails = Array.isArray(rawEmails) ? rawEmails.map((entry) => String(entry)) : [];
            await this.insertBulkUserContacts(userId, emails);
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Success!" });
        }
        catch {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Error inserting data" });
        }
    };
    delUser = async (request, response) => {
        try {
            ...
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "User Deleted Successfully", data: null });
        }
        catch {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Error while adding record in database" });
        }
    };
    rejectDual = async (request, response) => {
        try {
            const dualId = (0, legacy_request_1.getLegacyString)(request, "dual_id");
            const dualCaption = (0, legacy_request_1.getLegacyString)(request, "dual_caption");
            const dualPostedBy = (0, legacy_request_1.getLegacyString)(request, "dual_posted_by");
            const dualUserName = (0, legacy_request_1.getLegacyString)(request, "dual_user_name");
            const dualUpdatedBy = (0, legacy_request_1.getLegacyString)(request, "dual_updated_by");
            const dualDate = (0, legacy_request_1.getLegacyString)(request, "dual_date");
            const dualTime = (0, legacy_request_1.getLegacyString)(request, "dual_time");
            const dateTime = nowString();
            await this.exec(`UPDATE dualpost_master SET dual_caption='${escapeSql(dualCaption)}', dual_date='${escapeSql(dualDate)}', dual_time='${escapeSql(dualTime)}', date_time='${dateTime}', receiver_user_status = 2 WHERE dual_id='${escapeSql(dualId)}'`);
            ...
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Updated Successfully" });
        }
        catch {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Error while adding record in database" });
        }
    };
    playvideo = async (_request, response) => {
...
<object width="400" height="409" codebase="http://fpdownload.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=8,0,0,0">
  ...
  <embed height="500" width="650" flashvars="file=BigBuckBunny_115k.mov&amp;streamer=rtmp://184.72.239.149/vod&amp;rtmp.subscribe=false&amp;quality=best&amp;controlbar=bottom&amp;rtmp.tunneling=false&amp;frontcolor=#fff&amp;backcolor=#000&amp;stretching=exactfit&amp;autostart=true" wmode="transparent" allowfullscreen="true" allowscriptaccess="always" quality="high" src="http://s.zuuk.net/ply.swf" type="application/x-shockwave-flash"/>
    ...
    getComments = async (request, response) => {
        const bId = (0, legacy_request_1.getLegacyString)(request, "b_id");
        ...
        const live = await this.queryRow(`SELECT * FROM "liveStream_master" WHERE live_id=${Number(bId)}`);
        if (!live) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "2", message: "Live broadcast stopped by broadcaster." });
            return;
        }
        const rows = await this.queryRows(`SELECT t.user_name,t.user_id, t.user_email, t.user_pic, t.user_name, tr.* FROM user_master t, comments_master tr WHERE tr.broadcast_id='${escapeSql(bId)}' AND CAST(tr.comment_by AS INTEGER)=t.user_id ORDER BY tr.comment_id ASC`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Comments Found successfully", data: rows, viewerCount: live.viewersCount });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No comments found for this b_id", viewerCount: live.viewersCount });
    };
    postComment = async (request, response) => {
        try {
            const bId = (0, legacy_request_1.getLegacyString)(request, "broadcast_id");
            const commentBy = (0, legacy_request_1.getLegacyString)(request, "comment_by");
            const commentTxt = (0, legacy_request_1.getLegacyString)(request, "comment_txt");
            await this.exec(`INSERT INTO comments_master(comment_by, comment_txt, broadcast_id) values('${escapeSql(commentBy)}', '${escapeSql(commentTxt)}', '${escapeSql(bId)}')`);
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Comment Posted Successfully" });
        }
        catch {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Error while adding record in database" });
        }
    };
    postVenture = async (request, response) => {
        try {
            const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
            const ventureTitle = (0, legacy_request_1.getLegacyString)(request, "venture_title");
            const ventureUrl = (0, legacy_request_1.getLegacyString)(request, "venture_url");
            const thumbBase64 = (0, legacy_request_1.getLegacyString)(request, "venture_thumb");
            const fileName = `${(0, random_1.legacyRandomString)(12)}.jpg`;
            const diskPath = (0, storage_1.legacyStorageDiskPath)("venture", fileName);
            await (0, storage_1.writeBase64File)(diskPath, thumbBase64);
            const picpath = (0, storage_1.legacyStoragePublicUrl)(request, "venture", fileName);
            await this.exec(`INSERT INTO venture_master(venture_url, venture_posted_by, venture_thumb, venture_title) values('${escapeSql(ventureUrl)}', '${escapeSql(userId)}', '${escapeSql(picpath)}', '${escapeSql(ventureTitle)}')`);
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "User Updated Successfully" });
        }
        catch {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Error while adding record in database" });
        }
    };
    uploadVenture = async (request, response) => {
        try {
            const ventureVideo = (0, legacy_request_1.getLegacyOptionalString)(request, "venture_video");
            if (!ventureVideo) {
                (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "venture_video parameter is invalid" });
                return;
            }
            const buffer = Buffer.from(ventureVideo, "base64");
            const fileName = `${(0, random_1.legacyRandomString)(12)}.mp4`;
            const diskPath = (0, storage_1.legacyStorageDiskPath)("venture", fileName);
            await (0, storage_1.writeBase64File)(diskPath, ventureVideo);
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", url: (0, storage_1.legacyStoragePublicUrl)(request, "venture", fileName) });
        }
        catch {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "ErrorVideo upload" });
        }
    };
    getVentures = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const rows = await this.queryRows(`SELECT t.*, tr.* FROM user_master t, venture_master tr WHERE CAST(tr.venture_posted_by AS INTEGER)=t.user_id AND tr.is_reported='NO' AND (tr.venture_posted_by IN (SELECT following_id FROM followers_master WHERE followed_by='${escapeSql(userId)}') OR tr.venture_posted_by='${escapeSql(userId)}') ORDER BY tr.venture_id DESC LIMIT 100`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Venture Found successfully", data: rows });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No comments found for this b_id" });
    };
    getVentureDet = async (request, response) => {
        const ventureId = (0, legacy_request_1.getLegacyString)(request, "venture_id");
        const rows = await this.queryRows(`SELECT t.*, tr.* FROM user_master t, venture_master tr WHERE CAST(tr.venture_posted_by AS INTEGER)=t.user_id AND tr.venture_id='${escapeSql(ventureId)}'`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Venture Found successfully", data: rows });
            return;
        }
    ...
    getUserVenture = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const rows = await this.queryRows(`SELECT t.*, tr.* FROM user_master t, venture_master tr WHERE CAST(tr.venture_posted_by AS INTEGER)=t.user_id AND tr.venture_posted_by='${escapeSql(userId)}' AND tr.is_reported='NO' ORDER BY venture_id DESC`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Venture Found successfully", data: rows });
            return;
        }
    ...
    searchVenture = async (request, response) => {
        const searchTxt = (0, legacy_request_1.getLegacyString)(request, "search_txt");
        const rows = await this.queryRows(`SELECT t.*, tr.* FROM user_master t, venture_master tr WHERE CAST(tr.venture_posted_by AS INTEGER)=t.user_id AND tr.is_reported='NO' AND LOWER(tr.venture_title) LIKE LOWER('%${escapeSql(searchTxt)}%') LIMIT 100`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Venture Found successfully", data: rows });
            return;
        }
    ...
    searchLive = async (request, response) => {
        const searchTxt = (0, legacy_request_1.getLegacyString)(request, "search_txt");
        const rows = await this.queryRows(`SELECT t.*, tr.* FROM user_master t, "liveStream_master" tr WHERE CAST(tr.live_by AS INTEGER)=t.user_id AND tr.is_reported='NO' AND LOWER(tr.live_title) LIKE LOWER('%${escapeSql(searchTxt)}%') LIMIT 100`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Live Found successfully", data: rows });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No comments found for this b_id" });
    };
    searchDualPost = async (request, response) => {
        const searchTxt = (0, legacy_request_1.getLegacyString)(request, "search_txt");
        const rows = await this.queryRows(`SELECT * FROM dualpost_master WHERE is_reported='NO' AND LOWER(dual_caption) LIKE LOWER('%${escapeSql(searchTxt)}%') LIMIT 100`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Post Found successfully", data: rows });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No Duals found for this b_id" });
    };
    searchBusiness = async (request, response) => {
        const searchTxt = (0, legacy_request_1.getLegacyString)(request, "search_txt");
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const blockedRows = await this.queryRows(`SELECT user_id FROM blocks_master WHERE block_id='${escapeSql(userId)}'`);
        ...
        if (rows.length > 0) {
            for (const row of rows) {
                row.user_businesses = await this.userBusinesses(row.user_id);
            }
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "User Found successfully", data: rows });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No user found for the name" });
    };
    searchUser = async (request, response) => {
        const searchTxt = (0, legacy_request_1.getLegacyString)(request, "search_txt");
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const start = (0, legacy_request_1.getLegacyString)(request, "start", "0");
        const pageSize = (0, legacy_request_1.getLegacyString)(request, "page_size", "100");
        const blockedRows = await this.queryRows(`SELECT user_id FROM blocks_master WHERE block_id='${escapeSql(userId)}'`);
        const blockedIds = blockedRows.map((row) => Number(row.user_id)).filter((entry) => Number.isFinite(entry));
        const blockedCsv = blockedIds.join(",");
    ...
    searchUser2 = async (request, response) => {
        const searchTxt = (0, legacy_request_1.getLegacyString)(request, "search_txt");
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const start = (0, legacy_request_1.getLegacyString)(request, "start", "0");
        const pageSize = (0, legacy_request_1.getLegacyString)(request, "page_size", "100");
        const seed = (0, legacy_request_1.getLegacyOptionalString)(request, "seed") ?? String(Date.now());
        const blockedRows = await this.queryRows(`SELECT user_id FROM blocks_master WHERE block_id='${escapeSql(userId)}'`);
        const blockedRows2 = await this.queryRows(`SELECT block_id FROM blocks_master WHERE user_id='${escapeSql(userId)}'`);
        ...
        if (rows.length > 0) {
            ...
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "User Found successfully", data: rows, seed });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No user found for the name" });
    };
    getUserSuggession = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const start = (0, legacy_request_1.getLegacyString)(request, "start", "0");
        const pageSize = (0, legacy_request_1.getLegacyString)(request, "page_size", "100");
        const random = (0, legacy_request_1.getLegacyString)(request, "random", "0");
        const seed = (0, legacy_request_1.getLegacyOptionalString)(request, "seed") ?? String(Date.now());
        const followingRows = await this.queryRows(`SELECT following_id FROM followers_master WHERE followed_by='${escapeSql(userId)}'`);
        ...
        const randomUsers = async (offset, size) => {
        ...
        if (String(random) === "1") {
            ...
            if (data.length > 0) {
                (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "User Found successfully", data, seed, random_start: Number(start) + Number(pageSize) });
                return;
            }
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No user found for the name" });
            return;
        ...
        sql += ` LIMIT ${pageSize} OFFSET ${start}`;
        let rows = await this.queryRows(sql);
        for (const row of rows) {
            row.user_businesses = await this.userBusinesses(row.user_id);
        }
        ...
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "User Found successfully", data: rows, seed, random_start: rows.length });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No user found for the name" });
    };
    getFollowing = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const loggedUserId = (0, legacy_request_1.getLegacyOptionalString)(request, "logged_user_id") ?? userId;
        ...
        if (rows.length > 0) {
            const totalRow = await this.queryRow(`SELECT count(t.user_id) as total FROM user_master t, followers_master tr WHERE CAST(tr.following_id AS INTEGER)=t.user_id AND tr.followed_by='${escapeSql(userId)}'`);
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", data: rows, total: Number(totalRow?.total ?? 0) });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No data found" });
    };
    getFollowers = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const loggedUserId = (0, legacy_request_1.getLegacyOptionalString)(request, "logged_user_id") ?? userId;
        const { following, followers } = await this.buildFollowerList(userId, loggedUserId, false);
        if (following.length > 0) {
            const payload = { status: "1", following };
        ...
        if (followers.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", followers });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No data found" });
    };
    getFollowers2 = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const loggedUserId = (0, legacy_request_1.getLegacyOptionalString)(request, "logged_user_id") ?? userId;
        const start = (0, legacy_request_1.getLegacyOptionalString)(request, "start");
        const pageSize = (0, legacy_request_1.getLegacyOptionalString)(request, "page_size");
        ...
        if (rows.length > 0) {
            const totalRow = await this.queryRow(`SELECT count(distinct t.user_id) as total FROM user_master t, followers_master tr WHERE CAST(tr.followed_by AS INTEGER)=t.user_id AND tr.following_id='${escapeSql(userId)}'`);
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", data: rows, total: Number(totalRow?.total ?? 0) });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No data found" });
    ...
    getUserDetail = async (request, response) => {
        const username = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const isCount = Number((0, legacy_request_1.getLegacyOptionalString)(request, "is_count") ?? 0);
        ...
        (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "User details found successfully", data: payload });
    };
    getUserDetail5Jan2024 = async (request, response) => {
        const username = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const isCount = Number((0, legacy_request_1.getLegacyOptionalString)(request, "is_count") ?? 0);
        const result = await this.updateProfileViewCount(username, isCount);
        if (result === false) {
    ...
    updateProfile = async (request, response) => {
        try {
            ...
            const location = (0, legacy_request_1.getLegacyOptionalString)(request, "location");
            const userBio = (0, legacy_request_1.getLegacyOptionalString)(request, "user_bio");
            const userState = (0, legacy_request_1.getLegacyOptionalString)(request, "user_state");
            const hideAudience = (0, legacy_request_1.getLegacyOptionalString)(request, "hide_audience");
            const filename = (0, legacy_request_1.getLegacyOptionalString)(request, "fileName");
            let picPath = "";
            if (username) {
                const existing = await this.queryRow(`SELECT * FROM user_master WHERE user_name='${escapeSql(username.trim())}'`);
            ...
            if ((0, legacy_request_1.getLegacyOptionalString)(request, "profile_pic")) {
                const encoded = (0, legacy_request_1.getLegacyString)(request, "profile_pic");
                const fileName = `${(0, random_1.legacyRandomString)(10)}.jpg`;
                const diskPath = (0, storage_1.legacyStorageDiskPath)("profile", fileName);
                await (0, storage_1.writeBase64File)(diskPath, encoded);
                picPath = (0, storage_1.legacyStoragePublicUrl)(request, "profile", fileName);
            }
            const updateParts = [];
            if (username)
                updateParts.push(`user_name='${escapeSql(username.trim())}'`);
            if (location)
                updateParts.push(`user_location='${escapeSql(location)}'`);
            if (userBio)
                updateParts.push(`user_bio='${escapeSql(userBio)}'`);
            if (userState)
                updateParts.push(`user_state='${escapeSql(userState)}'`);
            if (hideAudience !== undefined)
                updateParts.push(`hide_audience='${escapeSql(hideAudience)}'`);
            if (picPath.length > 0)
                updateParts.push(`user_pic='${escapeSql(picPath)}'`);
            ...
            row.user_categories = await this.queryRows(`SELECT * FROM user_category where user_id='${escapeSql(userId)}'`).then((rows) => rows.map((entry) => entry.category_id));
            row.user_interests = await this.queryRows(`SELECT * FROM user_interest where user_id='${escapeSql(userId)}'`).then((rows) => rows.map((entry) => entry.interest_id));
            row.user_businesses = await this.queryRows(`SELECT * FROM user_business where user_id='${escapeSql(userId)}'`).then((rows) => rows.map((entry) => entry.business_id));
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "User Updated Successfully", data: row });
        }
        catch {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Error while adding record in database" });
        }
    };
    updateProfileViewCountRoute = async (_request, response) => {
        response.status(200).send("");
    };
    addVideoPost = async (request, response) => {
        try {
            const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
            const userName = (0, legacy_request_1.getLegacyString)(request, "user_name");
            const userPic = (0, legacy_request_1.getLegacyString)(request, "user_pic");
            const title = (0, legacy_request_1.getLegacyString)(request, "video_title");
            const cost = (0, legacy_request_1.getLegacyString)(request, "video_cost");
            const linkedTo = (0, legacy_request_1.getLegacyString)(request, "dual_linked_to", "-1");
            const linkedName = (0, legacy_request_1.getLegacyString)(request, "dual_linked_name", "NA");
            const linkedPic = (0, legacy_request_1.getLegacyString)(request, "dual_linked_propic", "NA");
            const dualDate = (0, legacy_request_1.getLegacyOptionalString)(request, "dual_date") ?? "";
            const dualTime = (0, legacy_request_1.getLegacyOptionalString)(request, "dual_time") ?? "";
            const receiverNotes = (0, legacy_request_1.getLegacyOptionalString)(request, "receiver_notes") ?? "";
            const receiverUserStatus = (0, legacy_request_1.getLegacyOptionalString)(request, "receiver_user_status") ?? "0";
            const collabPrice = (0, legacy_request_1.getLegacyOptionalString)(request, "collab_price") ?? "0";
            const cardId = (0, legacy_request_1.getLegacyOptionalString)(request, "card_id") ?? "";
            const taggedUserIds = (0, legacy_request_1.getLegacyOptionalString)(request, "tagged_user_ids") ?? "";
            const categoryIds = numericList((0, legacy_request_1.getLegacyOptionalString)(request, "category_ids") ?? "");
            const interestIds = numericList((0, legacy_request_1.getLegacyOptionalString)(request, "interest_ids") ?? "");
            ...
            let thumbUrl = "";
            if (thumbnail) {
                const thumbFileName = `${(0, random_1.legacyRandomString)(10)}.jpg`;
                const thumbPath = node_path_1.default.join(env_1.env.legacyThumbnailsRoot, thumbFileName);
                await promises_1.default.mkdir(env_1.env.legacyThumbnailsRoot, { recursive: true });
                await promises_1.default.writeFile(thumbPath, thumbnail.buffer);
                thumbUrl = `${request.secure ? "https" : "http"}://${request.headers.host ?? "localhost:3000"}/categories/thumbnails/${thumbFileName}`;
            }
            const videoExt = node_path_1.default.extname(video.originalname) || ".mov";
            const videoFileName = `${(0, random_1.legacyRandomString)(10)}${videoExt}`;
            const videoPath = node_path_1.default.join(env_1.env.legacyVideoPostsRoot, videoFileName);
            await promises_1.default.mkdir(env_1.env.legacyVideoPostsRoot, { recursive: true });
            await promises_1.default.writeFile(videoPath, video.buffer);
            const videoUrl = `${request.secure ? "https" : "http"}://${request.headers.host ?? "localhost:3000"}/categories/videoPosts/${videoFileName}`;
            const dateTime = nowString();
            const inserted = await this.queryRow(`INSERT INTO dualpost_master(dual_posted_by, dual_image, dual_caption, dual_posted_name, dual_posted_pic, video_url, video_cost, date_time, dual_type, dual_linked_to, dual_linked_name, dual_linked_profile_pic, dual_date, dual_time, receiver_notes, receiver_user_status, card_id, collab_price, payment_status, tagged_users)
         VALUES('${escapeSql(userId)}', '${escapeSql(thumbUrl)}', '${escapeSql(title)}', '${escapeSql(userName)}', '${escapeSql(userPic)}', '${escapeSql(videoUrl)}', '${escapeSql(cost)}', '${dateTime}', 2, '${escapeSql(linkedTo)}', '${escapeSql(linkedName)}', '${escapeSql(linkedPic)}', '${escapeSql(dualDate)}', '${escapeSql(dualTime)}', '${escapeSql(receiverNotes)}', '${escapeSql(receiverUserStatus)}', '${escapeSql(cardId)}', '${escapeSql(collabPrice)}', 'Pending', '${escapeSql(taggedUserIds)}') RETURNING *`);
            ...
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Video Created Successfully", data: inserted });
        }
        catch (error) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: `Error while adding record in database ${error instanceof Error ? error.message : ""}`.trim() });
        }
    };
    addVideoPostLegacy = async (request, response) => {
        await this.addVideoPost(request, response);
    };
    postDualPost = async (request, response) => {
        try {
            const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
            const userName = (0, legacy_request_1.getLegacyString)(request, "user_name");
            const userPic = (0, legacy_request_1.getLegacyString)(request, "user_pic");
            const title = (0, legacy_request_1.getLegacyString)(request, "dualP_title");
            const linkedTo = (0, legacy_request_1.getLegacyString)(request, "dual_linked_to");
            const dualDate = (0, legacy_request_1.getLegacyString)(request, "dual_date");
            const dualTime = (0, legacy_request_1.getLegacyString)(request, "dual_time");
            const linkedName = (0, legacy_request_1.getLegacyString)(request, "dual_linked_name", "NA");
            const linkedPic = (0, legacy_request_1.getLegacyString)(request, "dual_linked_propic", "NA");
            ...
            const categoryIds = numericList((0, legacy_request_1.getLegacyOptionalString)(request, "category_ids") ?? "");
            const interestIds = numericList((0, legacy_request_1.getLegacyOptionalString)(request, "interest_ids") ?? "");
            const profilePic = (0, legacy_request_1.getLegacyString)(request, "dual_pic");
            const base64 = Buffer.from(profilePic, "base64");
            const fileName = `${(0, random_1.legacyRandomString)(12)}.jpg`;
            const diskPath = (0, storage_1.legacyStorageDiskPath)("dualPosts", fileName);
            await promises_1.default.mkdir((0, storage_1.legacyStorageDiskPath)("dualPosts"), { recursive: true });
            await promises_1.default.writeFile(diskPath, base64);
            const picPath = (0, storage_1.legacyStoragePublicUrl)(request, "dualPosts", fileName);
            const dateTime = nowString();
            const row = await this.queryRow(`INSERT INTO dualpost_master(dual_image, dual_posted_by, dual_linked_to, dual_linked_name, dual_linked_profile_pic, dual_caption, dual_posted_name, dual_posted_pic, dual_date, dual_time, date_time, receiver_notes, receiver_user_status, card_id, collab_price, payment_status, tagged_users)
         VALUES('${escapeSql(picPath)}', '${escapeSql(userId)}', '${escapeSql(linkedTo)}', '${escapeSql(linkedName)}', '${escapeSql(linkedPic)}', '${escapeSql(title)}', '${escapeSql(userName)}', '${escapeSql(userPic)}', '${escapeSql(dualDate)}', '${escapeSql(dualTime)}', '${dateTime}', '${escapeSql(receiverNotes)}', '${escapeSql(receiverUserStatus)}', '${escapeSql(cardId)}', '${escapeSql(collabPrice)}', 'Pending', '${escapeSql(taggedUserIds)}') RETURNING *`);
            ...
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Posted Successfully" });
        }
        catch (error) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: `Error while adding record in database${error instanceof Error ? error.message : ""}` });
        }
    };
    updateDual = async (request, response) => {
        try {
            const dualId = (0, legacy_request_1.getLegacyString)(request, "dual_id");
            const dualCaption = (0, legacy_request_1.getLegacyString)(request, "dual_caption");
            const dualPostedBy = (0, legacy_request_1.getLegacyString)(request, "dual_posted_by");
            ...
            const dualTime = (0, legacy_request_1.getLegacyString)(request, "dual_time");
            const dateTime = nowString();
            const row = await this.queryRow(`SELECT * FROM dualpost_master WHERE dual_id='${escapeSql(dualId)}'`);
            if (!row) {
                (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Dual not found" });
                return;
            }
            const userDetails = await this.queryRow(`SELECT * FROM user_master WHERE user_id='${escapeSql(String(row.dual_linked_to ?? ""))}'`);
            const loginUserDetails = await this.queryRow(`SELECT * FROM user_master WHERE user_id='${escapeSql(String(row.dual_posted_by ?? ""))}'`);
            ...
            if (collabPrice > 0) {
                if (String(row.payment_status ?? "") === "Pending") {
                    if (!userDetails || !String(userDetails.stripe_account_id ?? "").length) {
                        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Dual user stripe account not setup!" });
                        return;
                    }
            ...
            await this.exec(`UPDATE dualpost_master SET dual_caption='${escapeSql(dualCaption)}', dual_date='${escapeSql(dualDate)}', dual_time='${escapeSql(dualTime)}', date_time='${dateTime}', receiver_user_status = 1 ${queryUpdate} WHERE dual_id='${escapeSql(dualId)}'`);
    ...
    updateDualLegacy = async (request, response) => {
        try {
            const dualId = (0, legacy_request_1.getLegacyString)(request, "dual_id");
            const dualCaption = (0, legacy_request_1.getLegacyString)(request, "dual_caption");
            const dualPostedBy = (0, legacy_request_1.getLegacyString)(request, "dual_posted_by");
            ...
            const dualTime = (0, legacy_request_1.getLegacyString)(request, "dual_time");
            const dateTime = nowString();
            await this.exec(`UPDATE dualpost_master SET dual_caption='${escapeSql(dualCaption)}', dual_date='${escapeSql(dualDate)}', dual_time='${escapeSql(dualTime)}', date_time='${dateTime}' WHERE dual_id='${escapeSql(dualId)}'`);
    ...
    delDualPost = async (request, response) => {
        try {
            const dualId = (0, legacy_request_1.getLegacyString)(request, "dualid");
            if (dualId.length <= 0) {
                (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Invalid Request" });
                return;
            }
            const row = await this.queryRow(`SELECT dual_id, dual_image, video_url, dual_type FROM dualpost_master WHERE dual_id=${Number(dualId)}`);
            if (row) {
                if (Number(row.dual_type ?? 0) === 2) {
                    if (String(row.dual_image ?? "").length > 0) {
                        await promises_1.default.rm(node_path_1.default.join(env_1.env.legacyThumbnailsRoot, node_path_1.default.basename(String(row.dual_image))), { force: true });
                    }
                    if (String(row.video_url ?? "").length > 0) {
                        await promises_1.default.rm(node_path_1.default.join(env_1.env.legacyVideoPostsRoot, node_path_1.default.basename(String(row.video_url))), { force: true });
                    }
                }
                else if (String(row.dual_image ?? "").length > 0) {
                    await promises_1.default.rm(node_path_1.default.join((0, storage_1.legacyStorageDiskPath)("dualPosts"), node_path_1.default.basename(String(row.dual_image))), { force: true });
                }
            }
            ...
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Deleted Successfully" });
        }
        catch {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "Error while adding record in database" });
        }
    };
    getSaveDuals = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const rows = await this.queryRows(`SELECT dp.*, CASE WHEN lm.loved_by IS NULL THEN FALSE ELSE TRUE END as is_loved, CASE WHEN sm.saved_by IS NULL THEN FALSE ELSE TRUE END as is_saved FROM dualpost_master dp LEFT JOIN save_master sm on dp.dual_id=sm.dual_id LEFT JOIN love_master lm on dp.dual_id=lm.dual_id and lm.loved_by=${Number(userId)} where sm.saved_by=${Number(userId)} order by sm.date_time DESC`);
        if (rows.length > 0) {
            for (const row of rows) {
                const loveCount = await this.queryRow(`SELECT count(*) AS loved_cnt FROM love_master WHERE dual_id=${Number(row.dual_id)}`);
    ...
    getLoveDuals = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const rows = await this.queryRows(`SELECT dp.*, CASE WHEN lm.loved_by IS NULL THEN FALSE ELSE TRUE END as is_loved FROM dualpost_master dp LEFT JOIN love_master lm on dp.dual_id=lm.dual_id where lm.loved_by=${Number(userId)} order by lm.date_time DESC`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Post Found successfully", data: rows });
            return;
        }
    ...
    getSuggestedVideos = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const followingRows = await this.queryRows(`SELECT following_id FROM followers_master WHERE followed_by='${escapeSql(userId)}'`);
        const followingCsv = followingRows.map((row) => String(row.following_id).trim()).filter((entry) => entry.length > 0).map((id) => `'${escapeSql(id)}'`).join(",");
        ...
        const rows = await this.queryRows(sql);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Videos Found successfully", data: rows });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No videos found for the name" });
    };
    getBrowse = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const categoryIds = (0, legacy_request_1.getLegacyString)(request, "category_ids");
        const start = (0, legacy_request_1.getLegacyString)(request, "start", "0");
        const pageSize = (0, legacy_request_1.getLegacyString)(request, "page_size", "100");
        ...
        if (rows.rows.length > 0) {
            ...
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Post Found successfully", data, total_posts: rows.total });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No Duals found" });
    };
    getBrowse2 = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const categoryIds = (0, legacy_request_1.getLegacyString)(request, "category_ids");
        const start = (0, legacy_request_1.getLegacyString)(request, "start", "0");
        const pageSize = (0, legacy_request_1.getLegacyString)(request, "page_size", "100");
        const type = (0, legacy_request_1.getLegacyString)(request, "type");
        const screen = (0, legacy_request_1.getLegacyString)(request, "screen");
        const seed = (0, legacy_request_1.getLegacyOptionalString)(request, "seed") ?? String(Date.now());
        const random = (0, legacy_request_1.getLegacyOptionalString)(request, "random") ?? "0";
        const rows = await this.dualFeedRows(userId, {
            categoryIds,
            start,
            pageSize,
        ...
        if (rows.rows.length > 0) {
            ...
            for (const row of rows.rows) {
                ...
                grouped[owner].dual_posts.push(row);
            }
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Post Found successfully", data: Object.values(grouped) });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No post found" });
    };
    getBrowse20032020 = async (request, response) => {
        await this.getBrowse2(request, response);
    };
    getPostByInterest = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const interestIds = (0, legacy_request_1.getLegacyString)(request, "interest_ids");
        ...
        if (rows.rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Post Found successfully", data: rows.rows, total_posts: rows.total, seed: rows.seed });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No Post found for this interest" });
    };
    getUserDuals = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const loggedUserId = (0, legacy_request_1.getLegacyOptionalString)(request, "logged_user_id");
        const start = (0, legacy_request_1.getLegacyOptionalString)(request, "start");
        const pageSize = (0, legacy_request_1.getLegacyOptionalString)(request, "page_size");
        const type = (0, legacy_request_1.getLegacyOptionalString)(request, "type") ?? "both";
        ...
        if (rows.length > 0) {
            let total = 0;
            if (start !== undefined && pageSize !== undefined) {
                const totalRow = await this.queryRow(sqlTotal);
                total = Number(totalRow?.total ?? 0);
            }
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Post Found successfully", data: rows, total });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No Duals found for this b_id" });
    };
    getUserDuals2 = async (request, response) => {
        await this.getUserDuals(request, response);
    };
    getDuals = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const rows = await this.queryRows(`SELECT dp.*, CASE WHEN lm.loved_by IS NULL THEN FALSE ELSE TRUE END as is_loved FROM dualpost_master dp LEFT JOIN love_master lm on dp.dual_id=lm.dual_id and lm.loved_by=${Number(userId)} WHERE dual_caption!='Not Provided' AND is_reported='NO' AND dual_type=1 AND (dual_posted_by IN (SELECT following_id FROM followers_master WHERE followed_by='${escapeSql(userId)}') OR dual_posted_by='${escapeSql(userId)}') ORDER BY date_time DESC LIMIT 100`);
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Post Found successfully", data: rows });
            return;
        }
    ...
    getDuals3 = async (request, response) => {
        ...
        if (rows.length > 0) {
            (0, legacy_response_1.sendLegacyJson)(response, { status: "1", message: "Dual Post Found successfully", data: rows, total_posts: rows.length, seed });
            return;
        }
        (0, legacy_response_1.sendLegacyJson)(response, { status: "0", message: "No Duals found for this b_id" });
    };
    getDuals4 = async (request, response) => {
        await this.getDuals3(request, response);
    };
    getDuals4Legacy = async (request, response) => {
        await this.getDuals3(request, response);
    };
    getPostFeed = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const type = (0, legacy_request_1.getLegacyString)(request, "type");
        const screen = (0, legacy_request_1.getLegacyString)(request, "screen");
        const start = (0, legacy_request_1.getLegacyOptionalString)(request, "start");
    ...
    getPostFeed2 = async (request, response) => {
        await this.getPostFeed(request, response);
    };
    getPostFeed25Jan2024 = async (request, response) => {
        const userId = (0, legacy_request_1.getLegacyString)(request, "user_id");
        const type = (0, legacy_request_1.getLegacyString)(request, "type");
        const screen = (0, legacy_request_1.getLegacyString)(request, "screen");
        const start = (0, legacy_request_1.getLegacyOptionalString)(request, "start");
    ...
    sendNotificationDev = async (request, response) => {
        await this.sendLegacyPush(request, true);
        response.type("html").send("<p>Message successfully delivered</p>");
    };
    test = async (_request, response) => {
        response.type("html").send("<html><body><pre>phpinfo()</pre></body></html>");
    };
    testCurl = async (_request, response) => {
        response.type("html").send("CURL is available on your web server\nNotifiation sent");
    };
    testWowza = async (_request, response) => {
        response.type("html").send("Required field(s) is missing");
    };
    testPush = async (_request, response) => {
        response.type("html").send("<p>Message successfully delivered</p>");
    };
    startstream = async (request, response) => {
        const streamName = (0, legacy_request_1.getLegacyOptionalString)(request, "stream_name");
        if (!streamName) {
            (0, legacy_response_1.sendLegacyJson)(response, { success: 0, message: "Required field(s) is missing" });
            return;
        }
        try {
    ...
    sqliteUsers = async (request, response) => {
        ...
        const requestedUsername = ((0, legacy_request_1.getLegacyOptionalString)(request, "username") ?? "").trim();
        const parsedLimit = Number((0, legacy_request_1.getLegacyOptionalString)(request, "limit") ?? "50");
        const limit = Number.isFinite(parsedLimit) && parsedLimit > 0 ? Math.min(parsedLimit, 500) : 50;
        const includeHash = ((0, legacy_request_1.getLegacyOptionalString)(request, "include_hash") ?? "1") !== "0";
        const columns = [
            "username",
            "real_name",
            "alternate_email",
        ...
        try {
            ...
            users = stdout.trim().length > 0 ? JSON.parse(stdout) : [];
        }
        catch (error) {
            response.status(500).json({
                status: 0,
        ...
        const submittedPassword = (0, legacy_request_1.getLegacyOptionalString)(request, "password");
        if (submittedPassword && requestedUsername.length > 0 && users.length === 1 && includeHash) {
            const storedHash = String(users[0].password_hash ?? "");
            try {
                const { stdout } = await execFileAsync("php", ["-r", "echo password_verify($argv[1], $argv[2]) ? '1' : '0';", submittedPassword, storedHash], { maxBuffer: 1024 * 1024 });
...
exports.LegacyCompatController = LegacyCompatController;
function requestIpSeed() {
    return "127001";
}
